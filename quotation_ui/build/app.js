(() => {
  ...
  const commonDefaults = {
    ...
    termAgility: 48,
    termFinance: 48,
    annualMileageAgility: 10000,
    rvPercentageAgility: 0,
  ...
  function masterModels() {
    return Array.isArray(masterData?.models)
      ? masterData.models
      : [];
  }

  function findMasterModel(name) {
    const models = masterModels();
    return models.find((model) => model.name === name) || null;
  }

  function applyMasterVehicleDefaults(data) {
    const models = masterModels();
    if (!models.length) return data;

    let model = findMasterModel(data.modelName);
    if (!model) {
      model = models[0];
      data.modelName = model.name;
    }

    const powertrains = Array.isArray(model.powertrains)
      ? model.powertrains
      : [];

    if (!powertrains.includes(data.powertrain)) {
      data.powertrain = powertrains[0] || "";
    }

    const prices = model.prices || {};
    if (Object.prototype.hasOwnProperty.call(prices, data.powertrain)) {
      data.exShowroomPrice = Number(prices[data.powertrain] || 0);
    } else {
      const firstPrice = Object.values(prices)[0];
      data.exShowroomPrice = Number(firstPrice || 0);
    }

    refreshRvInputs(data);
    return data;
  }

  function selectedRvRate(data = inputs) {
    if (!data) return null;
    const model = findMasterModel(data.modelName);
    if (!model) return null;

    const rates = model.rvRates || {};
    const byPowertrain = rates[data.powertrain] || {};
    const byMileage = byPowertrain[String(Number(data.annualMileageAgility || 10000))] || {};
    const rate = byMileage[String(Number(data.termAgility || 48))];
  ...
  function refreshRvInputs(data = inputs) {
    if (!data) return;
    const rate = selectedRvRate(data);
    data.rvPercentageAgility = rate === null ? 0 : rate;
  }
  ...
  function defaults(quotationType) {
    ...
    if (quotationType === "corporate") {
      ...
      result = { ...commonDefaults };
    }

    return applyMasterVehicleDefaults(result);
  }
  ...
  function calc(quotationType, data) {
    ...
    */
    ...
    const rvPercentageAgility = n("rvPercentageAgility");
    const buybackAgility = netExShowroom * rvPercentageAgility;
    ...
    const common = {
      ...
      rvPercentageAgility: roundValue(rvPercentageAgility),
      buybackAgility: roundValue(buybackAgility),
      balanceDealer: 0,
    };
  ...
  const modelSelect = () => {
    const models = masterModels();
    if (!models.length) {
      return `<select class="cell-input master-select" disabled><option>Master unavailable</option></select>`;
    }

    return `
      <select
        class="cell-input master-select"
        data-key="modelName"
      ...
      >
        ${models.map((model) => `
          <option
            ...
            ${inputs.modelName === model.name ? "selected" : ""}
          >${ESC(model.name)}</option>
        `).join("")}
      </select>
    `;
  };
  ...
  const powertrainSelect = () => {
    const model = findMasterModel(inputs.modelName);
    const options = model?.powertrains || [];
    if (!options.length) {
      return `<select class="cell-input master-select" disabled><option>-</option></select>`;
    }
    ...
    return `
      ...
      >
        ${options.map((value) => `
          <option
            value="${ESC(value)}"
            ${inputs.powertrain === value ? "selected" : ""}
          >${ESC(value)}</option>
        `).join("")}
      </select>
    `;
  ...
  const mileageSelect = () => {
    const options = [10000, 15000, 20000];
    return `
      <select
        class="cell-input tenure-select"
        data-key="annualMileageAgility"
        data-value-type="number"
      >
        ${options.map((value) => `
          <option
            value="${value}"
            ${Number(inputs.annualMileageAgility) === value ? "selected" : ""}
          >${new Intl.NumberFormat("en-IN").format(value)} Km</option>
        `).join("")}
      </select>
    `;
  };

  const lockedAmount = (value) => `
    <span class="master-value">${fmt(value)}</span>
  `;

  const rvPercentDisplay = () => {
    const rate = selectedRvRate();
    return `<span class="master-value rv-value">${rate === null ? "N/A" : `${(rate * 100).toFixed(2).replace(/\.00$/, "")}%`}</span>`;
  };
  ...
  function mainPricing() {
    ...
    html += priceRow({
      ...
      value:
        lockedAmount(inputs.exShowroomPrice),
    });
  ...
  function finance() {
    ...
    return `
      ...
      <table class="finance-table">
        <tbody>
          <tr>
            <td rowspan="12" class="category-cell finance-category">Finance</td>
            <td>Select Plan:</td>
            <th>STAR AGILITY+</th>
            <th>STAR FINANCE</th>
          ...
          <tr>
            <td>Downpayment</td>
            <td>${numberInput("downpaymentAgility")}</td>
            <td>${numberInput("downpaymentFinance")}</td>
            <td></td>
          </tr>

          <tr>
            <td>Loan Amount</td>
            <td>${numberInput("loanAgility")}</td>
            <td>${numberInput("loanFinance")}</td>
            <td></td>
          </tr>

          <tr>
            <td>Term (Months)</td>
            <td>${tenureSelect("termAgility", 60)}</td>
            <td>${tenureSelect("termFinance", 84)}</td>
            <td></td>
          </tr>

          <tr>
            <td>Annual Mileage (Kms)</td>
            <td>${mileageSelect()}</td>
            <td class="na-cell">Not applicable</td>
            <td></td>
          </tr>

          <tr>
            <td>RV Grid Percentage</td>
            <td class="calculated-amount finance-right">${rvPercentDisplay()}</td>
            <td class="na-cell">Not applicable</td>
            <td></td>
          </tr>
          ...
          <tr>
            <td>EMI (Arrears)</td>
            <td>${numberInput("emiAgility")}</td>
            <td>${numberInput("emiFinance")}</td>
            <td></td>
          </tr>

          <tr>
            <td>Rate of Interest</td>
            <td>${percentInput("roiAgility")}</td>
            <td>${percentInput("roiFinance")}</td>
            <td></td>
          </tr>

          <tr>
            <td>Buyback Value</td>
            <td class="calculated-amount finance-right">${calcSpan("buybackAgility")}</td>
            <td class="na-cell">Not applicable</td>
            <td></td>
          </tr>

          <tr>
            <td>Processing Fees &amp; Stamp Duty</td>
            <td>${numberInput("processingFeeAgility")}</td>
            <td>${numberInput("processingFeeFinance")}</td>
            <td></td>
          </tr>

          <tr>
            <td>Security Deposit (if applicable)</td>
            <td class="calculated-amount finance-right">${calcSpan("securityDepositAgility")}</td>
            <td class="calculated-amount finance-right"><span class="locked-zero">0</span></td>
            <td></td>
          </tr>

          <tr class="finance-total">
            <td>Net Disbursement Amount</td>
            <td class="finance-right">${calcSpan("netDisbursementAgility")}</td>
            <td class="finance-right">${calcSpan("netDisbursementFinance")}</td>
            <td></td>
          </tr>

          <tr>
            <td rowspan="4" class="category-cell finance-category">Exchange at MB Dealership</td>
            <td>Value of current vehicle:</td>
            <td>${numberInput("currentVehicleValue")}</td>
            <td></td><td></td>
          </tr>

          <tr>
            <td>Special Support by FP Name</td>
            <td>${numberInput("fpSupport")}</td>
            <td></td><td></td>
          </tr>

          <tr class="finance-total">
            <td>Total Value to be adjusted for customer (D)</td>
            <td>${calcSpan("adjustmentD")}</td>
            <td></td><td></td>
          </tr>

          <tr class="finance-total">
            <td>Net Outflow (A) + (B) + (C) -(D)</td>
            <td>${calcSpan("netOutflow")}</td>
            <td></td><td></td>
          </tr>
        </tbody>
      </table>
  ...
  function renderQuotation() {
    ...
    applyMasterVehicleDefaults(inputs);
    ...
    root
      ...
      .forEach((element) => {
        element.addEventListener("change", () => {
          ...
          const isString = element.dataset.valueType === "string";
          inputs[key] = isString ? element.value : Number(element.value);

          clearCachedPdf();

          if (key === "modelName") {
            const model = findMasterModel(inputs.modelName);
            const powertrains = model?.powertrains || [];
            inputs.powertrain = powertrains[0] || "";
            applyMasterVehicleDefaults(inputs);
            renderQuotation();
            return;
          }
          ...
          if (key === "termAgility" || key === "annualMileageAgility") {
            refreshRvInputs(inputs);
            renderQuotation();
            return;
          }
  ...
  function onRender(event) {
    ...
    masterData = args.master_data || masterData || { models: [] };
