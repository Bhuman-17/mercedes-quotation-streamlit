...
def header_block(inputs: dict[str, Any], quotation_type: str, quote_date: str, width: float) -> Table:
    ...
    vehicle = Paragraph(
        ...
        + "<b>Model Name:</b> " + str(inputs.get("modelName") or "")
...
def finance_table(inputs: dict[str, Any], calc: dict[str, Any], quotation_type: str, width: float) -> Table:
    ...
    mileage_value = inputs.get("annualMileageAgility") or 10000
    try:
        mileage_text = f"{int(float(mileage_value)):,} Km"
    except Exception:
        mileage_text = str(mileage_value)

    rv_rate = float(calc.get("rvPercentageAgility") or 0)
    rv_text = f"{rv_rate * 100:.2f}%".replace(".00%", "%") if rv_rate > 0 else "N/A"
    buyback_text: Any = calc.get("buybackAgility") if rv_rate > 0 else "N/A"
    ...
    rows = [
        ...
        ("Annual Mileage (Kms)", mileage_text, "Not applicable"),
        ("RV Grid Percentage", rv_text, "Not applicable"),
        ...
        ("Buyback Value", buyback_text, "Not applicable"),
    ...
    for i, (label, ag, fin) in enumerate(rows):
        ...
        ag_value = ag if isinstance(ag, str) else fmt(ag)
        fin_value = fin if isinstance(fin, str) else fmt(fin)
        data.append(
            [
                p("Finance", S_CENTER_BOLD) if is_header else "",
                p(label, S_BOLD if is_header or is_total else S_NORMAL),
                p(str(ag_value), S_CENTER_BOLD if is_header else S_RIGHT),
                p(str(fin_value), S_CENTER_BOLD if is_header else S_RIGHT),
                "",
            ]
        )
    ...
    # Editable/manual finance inputs have the grey Excel-style background.
    for r in (1, 2, 3, 4, 6, 7, 9, 10):
        style.append(("BACKGROUND", (2, r), (3, r), LIGHT_GREY))
    # RV %, Buyback, Security Deposit and Net Disbursement are calculated/read-only.
    for r in (5, 8, 10):
        style.append(("BACKGROUND", (2, r), (3, r), LIGHT_GREY))
    style.append(("BACKGROUND", (1, len(data) - 1), (3, len(data) - 1), DARK_GREY))
